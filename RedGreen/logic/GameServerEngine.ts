...
export class GameServerEngine {
  ...
  private botConfigs: Record<string, BotConfig> = {};
  
  private readonly GRACE_PERIOD_MS = 400;

  constructor() {
    this.state = this.getInitialState(GAME_CONFIG.INITIAL_BALANCE);
  ...
  public join(playerId: string, isBot: boolean = false, playerName?: string) {
    if (this.state.state !== GameState.LOBBY) return;

    if (!isBot && this.state.playerBalance >= GAME_CONFIG.ENTRY_FEE) {
        this.state.playerBalance -= GAME_CONFIG.ENTRY_FEE;
    } else if (!isBot && this.state.playerBalance < GAME_CONFIG.ENTRY_FEE) {
        this.state.playerBalance = 0; 
    }

    const newPot = this.state.pot + GAME_CONFIG.ENTRY_FEE;

    if (isBot) {
        this.botConfigs[playerId] = {
            reactionTime: 50 + Math.random() * 250, 
        };
    }
    ...
    // Генерируем простое имя для бота, если оно не передано
    let finalName = playerName;
    if (!finalName) {
        if (isBot) {
             const botNum = parseInt(playerId.split('_')[2]) || Math.floor(Math.random() * 100);
             finalName = `Bot ${botNum}`;
        } else {
             finalName = `Player ${playerId.slice(-4)}`;
        }
    }
    ...
    this.state = {
      ...
      players: {
        ...
        [playerId]: {
          id: playerId,
          name: finalName,
          x: (Math.random() - 0.5) * (GAME_CONFIG.FIELD_WIDTH - 4), 
          z: Math.random() * -2,
          color: color,
          isEliminated: false,
          hasFinished: false,
  ...
  public start() {
    ...
    let botCount = 0;
    while (Object.keys(this.state.players).length < GAME_CONFIG.TOTAL_BOTS + 1) {
       let botId = `bot_${Date.now()}_${botCount++}`;
       while(this.state.players[botId]) botId = `bot_${Date.now()}_${botCount++}`;
       this.join(botId, true);
    }
  ...
  public playerMove(playerId: string, dx: number, dz: number) {
    ...
    if (this.state.light === LightColor.RED) {
      ...
      if (timeSinceRed > this.GRACE_PERIOD_MS) {
          if (dx !== 0 || dz !== 0) {
            this.eliminatePlayer(playerId, 'MOVEMENT'); // Умер от движения
            return;
          }
      }
    }
    ...
    if (dx !== 0 || dz !== 0) {
      ...
      const moveX = (dx / length) * GAME_CONFIG.PLAYER_SPEED;
      const moveZ = (dz / length) * GAME_CONFIG.PLAYER_SPEED;
  ...
  public reset() {
    this.stopGameLoop();
    this.state = this.getInitialState(this.state.playerBalance);
    this.botConfigs = {};
    this.broadcast();
  }
  ...
  private finishPlayer(playerId: string) {
    if (this.state.winners.length >= GAME_CONFIG.MAX_WINNERS) {
        this.eliminatePlayer(playerId, 'OBSTACLE'); // Не успел = умер (условно, но без лазера)
        return;
    }
  ...
  private update(dt: number) {
    ...
    Object.values(this.state.players).forEach(p => {
      ...
      for (const obs of this.state.obstacles) {
        ...
        if (dist < hitRadius) {
            this.eliminatePlayer(p.id, 'OBSTACLE'); // Умер от препятствия
            break;
        }
      }
  ...
  private updateBot(bot: Player) {
    const config = this.botConfigs[bot.id];
    if (!config) return;

    const now = Date.now();
    const timeSinceSwitch = now - this.lastLightSwitchTime;

    let perceivedLight = this.state.light;
    
    if (timeSinceSwitch < config.reactionTime) {
        perceivedLight = this.state.light === LightColor.GREEN ? LightColor.RED : LightColor.GREEN;
    }

    if (perceivedLight === LightColor.GREEN) {
        let moveX = 0;
        let moveZ = 1;
        ...
        for (const obs of this.state.obstacles) {
            ...
            if (dz > 0 && dz < 18) {
                const dx = Math.abs(obs.x - bot.x);
                const safetyRadius = obs.radius + 2.0;

                if (dx < safetyRadius) {
                    if (obs.x > bot.x) moveX = -1; else moveX = 1;
                    if (dz < 8) {
                        if (obs.type === 'BALL') { moveZ = 0; moveX *= 2; } 
                        else { moveZ = 0.5; }
                    }
                }
            }
        }
        if (moveX === 0) moveX = (Math.random() - 0.5) * 0.2;

        this.playerMove(bot.id, moveX, moveZ);
    }
  }
  ...
  private endGame() {
    ...
    const winAmount = this.state.winners.length > 0 
        ? Math.floor(this.state.pot / this.state.winners.length) 
        : 0;
    
    this.state.winAmount = winAmount;

    // Ищем главного игрока в победителях (теперь по ID из приложения)
    // NOTE: В реальной реализации здесь будет проверка по сессии
    // Сейчас мы обновляем баланс только в локальном стейте
    this.state.winners.forEach(winnerId => {
         if (!winnerId.startsWith('bot_')) {
             this.state.playerBalance += winAmount;
         }
    });
