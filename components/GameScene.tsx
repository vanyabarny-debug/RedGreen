...
import { PerspectiveCamera, Environment, Text, Sparkles, Stars, Instance, Instances, Cylinder } from '@react-three/drei';
import * as THREE from 'three';
import { GameSchema, Player, LightColor, GAME_CONFIG, Obstacle } from '../types';
...
// Вернул старый, хороший звук лазера
const laserSound = new Howl({ src: ['https://assets.mixkit.co/active_storage/sfx/1666/1666-preview.mp3'], volume: 0.4 });

// Лес (оптимизированный через Instances)
const Forest = () => {
    const trees = useMemo(() => {
        const temp = [];
        for (let i = 0; i < 200; i++) {
            const isLeft = Math.random() > 0.5;
            const xOffset = (GAME_CONFIG.FIELD_WIDTH / 2) + 5 + Math.random() * 40;
            const x = isLeft ? -xOffset : xOffset;
            const z = Math.random() * (GAME_CONFIG.FIELD_LENGTH + 40) - 20;
...
const LaserManager = ({ players }: { players: Record<string, Player> }) => {
    const [lasers, setLasers] = useState<{ id: string, target: THREE.Vector3 }[]>([]);
    const eliminatedIds = useRef<Set<string>>(new Set());
    const cannonPosition = new THREE.Vector3(0, 14, GAME_CONFIG.FIELD_LENGTH + 5); 
    ...
    useFrame(() => {
        ...
        Object.values(players).forEach(p => {
            if (p.isEliminated && !eliminatedIds.current.has(p.id)) {
                ...
                // Стреляем только если причина - движение (или не указана)
...
const Cannon = () => {
    return (
        <group position={[0, 14, GAME_CONFIG.FIELD_LENGTH + 5]} rotation={[0.3, 0, 0]}>
            {/* Base */}
            <mesh position={[0, 0, 0]}>
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      ...
      if (player.isEliminated) {
        ...
        // Исправлена логика доступа к модели для перекраски
        // meshRef.current -> LegoMan Group -> Torso Mesh (index 2)
        if (meshRef.current.children[0] && meshRef.current.children[0].children[2]) {
             ...
             if (player.deathReason === 'OBSTACLE') {
                ...
                if (material) material.color.set('#000000'); // Ожог (АБСОЛЮТНО ЧЕРНЫЙ)
             }
        }
    ...
    if (arrowRef.current && isMe) {
        arrowRef.current.position.y = 3.8 + Math.sin(state.clock.elapsedTime * 6) * 0.3;
  ...
  return (
    <group ref={meshRef} position={[player.x, 0, player.z]}>
      ...
      {isMe && !player.isEliminated && (
          <>
            <pointLight position={[0, 2, 0]} distance={10} intensity={2} color="#fbbf24" />
            <mesh position={[0, 0.1, 0]} rotation={[-Math.PI/2, 0, 0]}>
                <ringGeometry args={[0.6, 0.8, 32]} />
                <meshBasicMaterial color="#fbbf24" opacity={0.6} transparent />
            </mesh>
            <group ref={arrowRef} position={[0, 3.8, 0]}>
      ...
      {!player.isEliminated && (
          <Text
            ...
            outlineColor="black"
          >
            {isMe ? "ВЫ" : (player.name || `P-${player.id.split('_')[2] || player.id.slice(-2)}`)}
          </Text>
      )}
    </group>
  );
...
const TrafficLight = ({ light, timeRemaining }: { light: LightColor, timeRemaining: number }) => {
    const isGreen = light === LightColor.GREEN;
    return (
        <group position={[0, 8, GAME_CONFIG.FIELD_LENGTH + 5]}>
            <mesh position={[-3, 0, 0]}>
...
const GameContent = ({ gameState, playerId, onMove }: SceneProps) => {
    ...
    useFrame((state, delta) => {
        ...
        if (!MyPlayer || MyPlayer.isEliminated || MyPlayer.hasFinished) return;
        
        const targetZ = MyPlayer.z;
        state.camera.position.lerp(new THREE.Vector3(0, 15, targetZ - 18), 0.1);
    ...
    return (
        <>
            ...
            <Sparkles count={500} scale={[GAME_CONFIG.FIELD_WIDTH + 20, 20, GAME_CONFIG.FIELD_LENGTH]} size={2} speed={0.4} opacity={0.5} color="#fbbf24" />

            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 50]} receiveShadow>
                <planeGeometry args={[GAME_CONFIG.FIELD_WIDTH + 80, 400]} />
                <meshStandardMaterial color="#1c1917" roughness={0.9} />
            </mesh>
            
            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.01, 50]} receiveShadow>
                <planeGeometry args={[GAME_CONFIG.FIELD_WIDTH, 400]} />
                <meshStandardMaterial color="#44403c" />
            </mesh>

            <Forest />

            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.02, GAME_CONFIG.FIELD_LENGTH]} receiveShadow>
                <planeGeometry args={[GAME_CONFIG.FIELD_WIDTH, 2]} />
                <meshStandardMaterial color="#facc15" emissive="#facc15" emissiveIntensity={0.5} />
            </mesh>
            <Text position={[0, 4, GAME_CONFIG.FIELD_LENGTH]} rotation={[0, Math.PI, 0]} fontSize={4} color="#facc15" outlineWidth={0.1} outlineColor="black">
                FINISH
            </Text>

            <TrafficLight light={gameState.light} timeRemaining={gameState.timeRemaining} />
            
            <Cannon />
            
            <LaserManager players={gameState.players} />
