...
import { Trophy, Crown, Medal, Lock, Users, Globe, CalendarRange, Activity } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 pb-10">
        <div className="flex justify-between items-center">
             <h2 className="text-2xl font-bold font-mono text-rpg-text">Боевая Аналитика</h2>
             <div className="flex bg-rpg-card rounded border border-rpg-border p-1">
                 {(['week', 'month', '3months', 'all'] as TimeRange[]).map(r => (
                     <button 
                        key={r} 
                        onClick={() => setRange(r)}
                        className={`px-3 py-1 text-xs font-bold uppercase rounded transition-colors ${range === r ? 'bg-rpg-primary text-rpg-bg' : 'text-gray-500 hover:text-rpg-text'}`}
                     >
        ...
        <div className="bg-rpg-panel p-5 rounded border border-rpg-border shadow-lg">
             <div className="flex items-center gap-2 mb-4 border-b border-rpg-border pb-2">
                 ...
                 <h3 className="font-bold text-rpg-text">Временные Шкалы</h3>
             </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-rpg-panel p-5 rounded border border-rpg-border shadow-lg flex items-center justify-between">
                <div>
                     ...
                     <div className="space-y-1 text-xs">
                         {pieData.map(d => (
                             <div key={d.name} className="flex items-center gap-2">
                                 ...
                                 <span className="text-gray-400">{d.name}: <span className="text-rpg-text font-bold">{d.value}</span></span>
                             </div>
                         ))}
                     </div>
                 </div>
                     ...
                     <div className="flex-1 overflow-y-auto max-h-16 custom-scrollbar text-[10px] space-y-1">
                         {skillData.map(d => (
                             <div key={d.name} className="flex justify-between">
                                 ...
                                 <span className="text-rpg-text">{d.value}</span>
                             </div>
                         ))}
                     </div>
                </div>
        ...
        {/* Mood Chart (New) */}
        <div className="bg-rpg-panel p-5 rounded border border-rpg-border shadow-lg flex flex-col">
        ...
        {/* Leaderboard Section */}
        <div className="bg-rpg-panel rounded-lg border border-rpg-border shadow-lg overflow-hidden">
            <div className="p-4 border-b border-rpg-border flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <Trophy className="text-yellow-500" />
                    <h3 className="text-lg font-bold text-rpg-text font-mono uppercase tracking-widest">
                ...
                <div className="flex bg-rpg-card rounded border border-rpg-border p-1">
                    <button 
                        ...
                        className={`px-3 py-1 text-xs font-bold uppercase rounded flex items-center gap-2 transition-colors ${lbMode === 'global' ? 'bg-rpg-primary text-rpg-bg' : 'text-gray-500 hover:text-rpg-text'}`}
                    >
                        <Globe size={14}/> Мир
                    </button>
                    <button 
                        onClick={() => setLbMode('friends')}
                        className={`px-3 py-1 text-xs font-bold uppercase rounded flex items-center gap-2 transition-colors ${lbMode === 'friends' ? 'bg-rpg-secondary text-white' : 'text-gray-500 hover:text-rpg-text'}`}
                    >
                    ...
                    <tbody>
                        ...
                        {displayedLeaderboard.map((player) => (
                            ...
                            >
                                ...
                                <td className={`px-6 py-4 font-bold flex items-center gap-3 ${player.isUser ? 'text-rpg-secondary' : 'text-rpg-text'}`}>
                                    {/* Avatar Display */}
