...
import { Scroll, CheckCircle, Flame, PlusCircle, AlertTriangle, XCircle, Skull, Users, Share2, Plus } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
           <h2 className="text-3xl font-bold text-white flex items-center gap-3 tracking-tight">
               <Scroll className="text-white"/> Квесты
           </h2>
           <div className="flex bg-rpg-card border border-rpg-border rounded-lg p-1">
               <button onClick={() => { playSound('click'); setTab('board');}} className={`px-4 py-2 rounded-md text-sm font-bold transition-colors ${tab === 'board' ? 'bg-white text-black shadow-sm' : 'text-gray-400 hover:text-white'}`}>Доска</button>
               <button onClick={() => { playSound('click'); setTab('custom');}} className={`px-4 py-2 rounded-md text-sm font-bold transition-colors ${tab === 'custom' ? 'bg-white text-black shadow-sm' : 'text-gray-400 hover:text-white'}`}>Создать +</button>
           </div>
      </div>
      ...
      {tab === 'board' ? (
        <>
            {/* Active Quest */}
            <section className="bg-rpg-card p-6 rounded-xl border border-rpg-border relative overflow-hidden group shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {availableQuests.map(quest => (
                    <div key={quest.id} className="bg-rpg-panel p-5 rounded-xl border border-rpg-border hover:border-white transition-all hover:shadow-lg group flex flex-col justify-between">
                        <div>
                            <div className="flex justify-between items-start mb-2">
                                <h4 className="font-bold text-lg text-white group-hover:text-rpg-primary transition-colors">{quest.title}</h4>
                        ...
                        <div className="flex items-center justify-between mt-auto pt-4 border-t border-rpg-border/50">
                            ...
                            <button 
                                ...
                                className={`px-4 py-2 rounded text-xs font-bold uppercase tracking-wider transition-colors ${activeQuest ? 'bg-rpg-border text-gray-400 cursor-not-allowed' : 'bg-rpg-primary text-black hover:bg-white'}`}
                            >
                                Принять
                            </button>
                        </div>
          ...
          <div className="max-w-2xl mx-auto bg-rpg-panel border border-rpg-border p-6 md:p-8 rounded-xl shadow-2xl">
              ...
              <form onSubmit={handleCreateCustom} className="space-y-6">
                  <div>
                      ...
                      <input className="w-full bg-rpg-card border border-rpg-border p-3 rounded text-white focus:border-rpg-primary outline-none" value={customTitle} onChange={e => setCustomTitle(e.target.value)} required placeholder="Например: Марафон Кодинга"/>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div>
                          <label className="block text-gray-500 text-xs font-mono uppercase mb-2">Цель: Кол-во задач</label>
                          <input type="number" min="1" className="w-full bg-rpg-card border border-rpg-border p-3 rounded text-white focus:border-rpg-primary outline-none" value={reqCount} onChange={e => setReqCount(Number(e.target.value))} />
                      </div>
                      <div>
                          <label className="block text-gray-500 text-xs font-mono uppercase mb-2">Требуемый навык</label>
                          <select className="w-full bg-rpg-card border border-rpg-border p-3 rounded text-white focus:border-rpg-primary outline-none" value={reqSkill} onChange={e => setReqSkill(e.target.value)}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                           ...
                           <input type="number" min="0" className="w-full bg-rpg-card border border-rpg-border p-3 rounded text-white focus:border-rpg-primary outline-none" value={betXP} onChange={e => setBetXP(Number(e.target.value))} placeholder="0"/>
                      ...
                      <div>
                           ...
                           <input type="date" className="w-full bg-rpg-card border border-rpg-border p-3 rounded text-white focus:border-rpg-primary outline-none" value={deadline} onChange={e => setDeadline(e.target.value)} />
                      </div>
                  </div>
                  <button type="submit" className="w-full bg-rpg-primary text-black font-bold py-3 rounded uppercase tracking-wider shadow hover:bg-white transition-colors">
                      Инициализировать
                  </button>
              </form>
          </div>
