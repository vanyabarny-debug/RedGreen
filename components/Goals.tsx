  ...
  return (
    <div className="space-y-6">
        <div className="flex justify-between items-center">
            ...
            <button onClick={() => setModalOpen(true)} className="bg-white text-black hover:bg-gray-200 px-4 py-2 rounded font-bold flex items-center gap-2 text-sm">
                <Plus size={16} /> Новая
            </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {goals.map(goal => {
                ...
                return (
                    <div key={goal.id} className={`bg-rpg-panel p-6 rounded-lg border ${goal.completed ? 'border-rpg-success/50 opacity-40 order-last' : 'border-rpg-border hover:border-white/50'} transition-all group shadow-sm flex flex-col`}>
                        ...
                        <div className="flex justify-between items-center pt-4 border-t border-rpg-border mt-auto">
                             ...
                             <button 
                                ...
                                className={`px-6 py-2 rounded font-bold flex items-center gap-2 transition-all text-sm ${goal.completed ? 'bg-rpg-success text-black' : 'bg-rpg-card text-gray-300 hover:bg-white hover:text-black'}`}
                            >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                <form onSubmit={handleAddGoal} className="bg-rpg-panel w-full max-w-lg p-8 rounded-lg border border-rpg-border shadow-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="space-y-4">
                        <input className="w-full bg-rpg-card border border-rpg-border rounded p-3 text-white focus:border-white outline-none" placeholder="Название" value={title} onChange={e => setTitle(e.target.value)} required/>
                        <textarea className="w-full bg-rpg-card border border-rpg-border rounded p-3 text-white h-24 resize-none focus:border-white outline-none" placeholder="Описание" value={desc} onChange={e => setDesc(e.target.value)}/>
                        <div className="grid grid-cols-2 gap-4">
                             <div>
                                 <label className="text-xs text-gray-500 mb-1 block uppercase font-mono">Награда XP (Max 200)</label>
                                 <input type="number" max="200" className="w-full bg-rpg-card border border-rpg-border rounded p-3 text-white focus:border-white outline-none" value={xp} onChange={e => setXp(Number(e.target.value))}/>
                             </div>
                             <div>
                                 <label className="text-xs text-gray-500 mb-1 block uppercase font-mono">Связанный навык</label>
                                 <select className="w-full bg-rpg-card border border-rpg-border rounded p-3 text-white focus:border-white outline-none" value={skill} onChange={e => setSkill(e.target.value)} required>
                        ...
                        <div className="border-t border-rpg-border pt-4">
                             ...
                             <div className="flex flex-wrap gap-2 mb-3">
                                 {[
                                     ...
                                     <button 
                                        ...
                                        className="bg-rpg-card border border-rpg-border text-xs text-gray-400 hover:text-white hover:border-white px-2 py-1 rounded"
                                     >
                                         {p.l}
                                     </button>
                                 ))}
                             ...
                             <div className="grid grid-cols-2 gap-4">
                                 <div>
                                     <input type="number" min="0" className="w-full bg-rpg-card border border-rpg-border rounded p-3 text-white focus:border-white outline-none" value={daysDur} onChange={e => setDaysDur(Number(e.target.value))}/>
                                     <span className="text-[10px] text-gray-500 uppercase">Дней</span>
                                 </div>
                                 <div>
                                     <input type="number" min="0" className="w-full bg-rpg-card border border-rpg-border rounded p-3 text-white focus:border-white outline-none" value={hoursDur} onChange={e => setHoursDur(Number(e.target.value))}/>
