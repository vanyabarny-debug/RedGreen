...
const Inventory: React.FC = () => {
  ...
  const activeList = state.inventory.find(l => l.id === activeListId);
  ...
  // Helper to render link content based on type
  const renderLinkContent = (url: string) => {
      ...
      // Generic Link Card (Telegram/Notion Style)
      let domain = '';
        ...
        {!activeListId && (
            <div className="flex items-center justify-between mb-6 shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-3xl font-bold text-white tracking-tight">Склад</h2>
                </div>
            </div>
        )}

        {/* View 1: Grid of Backpacks */}
        {!activeListId && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 overflow-y-auto pb-4">
                {/* CREATE CARD */}
                <button 
                    onClick={() => setIsCreatingList(true)}
                    className="bg-rpg-panel border-2 border-dashed border-rpg-border rounded-xl p-6 flex flex-col items-center justify-center text-gray-500 hover:text-white hover:border-white hover:bg-rpg-card transition-all min-h-[180px] group"
                >
                ...
                {state.inventory.map(list => (
                    <div 
                        ...
                        className="bg-rpg-panel border border-rpg-border hover:border-white hover:bg-rpg-card/50 rounded-xl p-5 cursor-pointer transition-all group relative flex flex-col min-h-[180px]"
                    >
                        <div className="flex justify-between items-start mb-3">
                            <div className="p-2 bg-rpg-card rounded-full text-white border border-rpg-border group-hover:border-white transition-colors">
                                <Backpack size={20}/>
                            </div>
                            <button 
                        ...
                        <div className="flex-1 overflow-hidden space-y-1 mb-2">
                            {list.items.slice(0, 4).map(item => (
                                <div key={item.id} className="text-[10px] text-gray-400 truncate flex items-center gap-1">
                                    <div className={`w-1 h-1 rounded-full ${item.checked ? 'bg-gray-600' : 'bg-white'}`}></div>
                                    <span className={item.checked ? 'line-through opacity-50' : ''}>{item.text}</span>
                                </div>
                            ))}
                            {list.items.length === 0 && <span className="text-[10px] text-gray-600 italic">Пусто</span>}
                            {list.items.length > 4 && <span className="text-[10px] text-gray-500 italic">...и еще {list.items.length - 4}</span>}
                        </div>

                        <div className="text-xs text-gray-500 font-mono border-t border-rpg-border pt-2 flex justify-between items-center">
                            <span>Предметов:</span>
