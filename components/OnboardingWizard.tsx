...
import { Target, ArrowRight, Brain, Clock, Check, Plus, ChevronDown, ChevronRight, X, Coffee, Calendar, Hourglass, Zap, Layers, Network, Shield, Sword, LayoutList } from 'lucide-react';
    ...
    return (
        <div className="mb-2 animate-fade-in">
            <div className="flex items-center gap-2" style={{ marginLeft: `${indent * 4}px` }}>
                ...
                <input 
                    className={`bg-transparent border-b border-transparent hover:border-gray-700 focus:border-rpg-primary outline-none transition-colors px-1 py-0.5 text-white placeholder-gray-600 ${node.type === 'root' ? 'text-xl font-bold text-yellow-500' : node.type === 'pillar' ? 'text-lg font-bold' : 'text-sm'}`}
                    value={node.name}
                ...
                <div className="flex items-center gap-1">
                    ...
                    {node.type !== 'root' && (
                        <button onClick={() => deleteNode(node.id)} className="text-red-500 hover:bg-red-500/10 p-1 rounded transition-colors">
                            <X size={14}/>
                        </button>
                    )}
                </div>
...
const OnboardingWizard: React.FC = () => {
    ...
    // Initialize step based on user state to support resuming or forced flows (e.g. from settings)
    ...
    const finishOnboarding = () => {
        ...
        if (!blueprint) {
            console.error("Blueprint is missing!");
            return;
        }
        ...
        // 2. Recursively convert Blueprint to Skills
        // Changed logic: All nodes inherit the final deadline of the main goal.
        const processNode = (node: BlueprintNode, parentId: string | null, inheritedColor: string, depth: number) => {
            ...
            const newSkill: Skill = {
                ...
                currentXP: 0,
                maxXP: maxXP,
                deadline: finalDeadline.toISOString(), // Inherited deadline
        ...
        try {
            ...
            console.error("[Tree Builder] Critical Error:", e);
        }
    };

    return (
        <div className="fixed inset-0 z-[200] bg-black/95 flex flex-col items-center justify-center p-4 text-white">
            <div className="w-full max-w-3xl bg-rpg-panel border border-rpg-border rounded-xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
                ...
                {step === 0 && (
                    <div className="p-8 md:p-12 flex flex-col items-center text-center animate-fade-in h-full justify-center">
                        ...
                        <h2 className="text-3xl md:text-4xl font-bold font-mono text-white mb-4">Добро пожаловать</h2>
                ...
                {step === 1 && (
                    <div className="p-6 h-full flex flex-col justify-center animate-fade-in overflow-y-auto">
                        <h2 className="text-2xl md:text-3xl font-bold text-center mb-6 font-mono">Выберите Режим</h2>
                ...
                {selectedModeInfo && (
                    <div className="absolute inset-0 bg-black/95 z-50 flex items-center justify-center p-6 animate-fade-in">
                        <div className="w-full max-w-lg bg-rpg-panel border border-rpg-border rounded-xl p-6 relative">
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="text-left text-sm text-gray-300 space-y-3 bg-rpg-card p-4 rounded mb-6 border border-rpg-primary/30">
                                        ...
                                        <ul className="space-y-2">
                                            ...
                                            <li className="flex items-center gap-2"><Clock size={14} className="text-yellow-500"/> Требует 5-10 минут на настройку</li>
                                        </ul>
                                    </div>
                                    <button 
                                        onClick={() => {
                                            ...
                                            setStep(2); // Go to Warning Step
                                        }}
                                        className="w-full py-3 bg-rpg-primary text-black font-bold rounded hover:opacity-90 transition-colors"
                                    >
                                        Выбрать Продвинутый
                                    </button>
                                </div>
                ...
                {/* Step 2: Advanced Warning (Only for Advanced Path) */}
                {step === 2 && (
                    <div className="p-8 md:p-12 flex flex-col items-center text-center animate-fade-in h-full justify-center">
                        <Shield size={80} className="text-rpg-primary mb-6 animate-pulse"/>
                        <h2 className="text-3xl md:text-4xl font-bold font-mono text-white mb-4">Архитектура Личности</h2>
                        <div className="bg-rpg-card border border-rpg-border p-6 rounded-lg max-w-xl text-left space-y-4 mb-8">
                            <p className="text-white leading-relaxed font-medium">
                        ...
                        <button 
                            onClick={() => setStep(3)}
                            className="bg-rpg-primary text-black font-bold py-3 px-10 rounded hover:brightness-110 transition-transform hover:scale-105 uppercase tracking-widest"
                        >
                            Я готов
                        </button>
                    </div>
                ...
                {step === 3 && (
                    <div className="p-8 flex flex-col items-center justify-center h-full animate-fade-in">
                        <Target size={64} className="text-rpg-primary mb-6"/>
                        <h3 className="text-2xl font-bold text-center mb-2 text-white">Главная Цель</h3>
                        <p className="text-gray-400 text-center mb-8">Ваша Полярная Звезда.</p>
                        <input 
                            className="w-full max-w-lg bg-rpg-card border border-rpg-border p-4 rounded text-xl text-center text-white focus:border-rpg-primary outline-none mb-8 placeholder-gray-600"
                        ...
                        <button 
                            ...
                            disabled={!mainGoal}
                            className={`px-8 py-3 rounded font-bold transition-all ${mainGoal ? 'bg-rpg-primary text-black hover:scale-105' : 'bg-gray-800 text-gray-500'}`}
                        >
                ...
                {/* Step 4: Deadline */}
                {step === 4 && (
                    <div className="p-4 md:p-8 flex flex-col items-center justify-center h-full animate-fade-in overflow-y-auto">
                        <Hourglass size={64} className="text-white mb-4 md:mb-6 shrink-0"/>
                        <h3 className="text-xl md:text-2xl font-bold text-center mb-4 text-white shrink-0">Временной Горизонт</h3>
                        
                        <div className="mb-8 max-w-lg text-center bg-gradient-to-r from-transparent via-rpg-primary/10 to-transparent p-4 rounded-lg shrink-0">
                            <p className="text-white font-serif italic text-sm md:text-lg leading-relaxed">
                                "При должной дисциплине и усердии, года более чем достаточно, чтобы изменить жизнь до неузнаваемости."
                            </p>
                        </div>

                        {/* Mobile: Grid 2 cols, Desktop: Grid 5 cols */}
                        ...
                        <button 
                            ...
                            disabled={!years}
                            className={`px-8 py-3 rounded font-bold transition-all uppercase tracking-widest shrink-0 ${years ? 'bg-white text-black hover:bg-gray-200' : 'bg-gray-800 text-gray-600 cursor-not-allowed'}`}
                        >
                            Далее
                        </button>
                    </div>
                ...
                {step === 5 && (
                    <div className="p-8 flex flex-col items-center justify-center h-full animate-fade-in">
                        ...
                        <h3 className="text-2xl font-bold text-center mb-2">Опоры Цели</h3>
                        ...
                        <button 
                            ...
                            className="bg-rpg-primary text-black font-bold py-3 px-8 rounded hover:bg-white transition-all"
                        >
                ...
                {step === 6 && blueprint && (
                    <div className="flex flex-col h-full animate-fade-in">
                        <div className="p-6 border-b border-rpg-border bg-rpg-card flex justify-between items-center shrink-0">
                            <div>
                                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                                    <Clock size={20} className="text-rpg-primary"/> Карта Целей
                                </h3>
                                <p className="text-xs text-gray-400 mt-1">
                                    Детализируйте план. Добавьте конкретные шаги.
                                </p>
                            </div>
                            <button 
                                onClick={finishOnboarding}
                                className="bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded font-bold shadow-lg shadow-green-900/50 transition-all flex items-center gap-2"
                            >
                        ...
                        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-rpg-bg">
                            <div className="max-w-3xl mx-auto">
                                <BuilderNode 
                                    ...
                                    deleteNode={deleteNode}
                                />
                                <div className="mt-12 p-4 border border-dashed border-gray-700 rounded text-center text-gray-500 text-sm">
                                    <p>Вы всегда сможете отредактировать это дерево позже.</p>
                                </div>
                            </div>
                        </div>
                    </div>
