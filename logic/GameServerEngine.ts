...
const APP_ID = 'rlgl-game-v1';

type NetworkAction = 
  | { type: 'HOST_STATE', state: Partial<GameSchema> }
  | { type: 'CLIENT_JOIN', player: Player }
  | { type: 'CLIENT_MOVE', playerId: string, x: number, z: number }
  | { type: 'CLIENT_LEAVE', playerId: string };
...
export class GameServerEngine {
  ...
  private onAction: any = null;
  private peers: Record<string, any> = {};

  // Global Lobby Room (for finding games)
  ...
  constructor() {
    ...
    // Initialize Global Lobby to find games
    this.connectToLobby();
  }

  private connectToLobby() {
      try {
        this.lobbyRoom = joinRoom({ appId: APP_ID }, 'global-lobby');
        const [sendRoomInfo, getRoomInfo] = this.lobbyRoom.makeAction('roomInfo');
        
        getRoomInfo((data: RoomInfo, peerId: string) => {
            // Only add if not exists and valid
            if (!this.state.roomsList.find(r => r.id === data.id)) {
                this.state.roomsList = [...this.state.roomsList, data];
                this.broadcast();
            }
        });

        // Periodically announce my room if I am host
        setInterval(() => {
            if (this.isHost && this.state.roomId) {
                const myRoom = this.state.roomsList.find(r => r.id === this.state.roomId);
                if (myRoom) sendRoomInfo(myRoom);
            }
        }, 3000);
      } catch (e) {
          console.warn("P2P Lobby connection failed", e);
      }
  }
  ...
  public createRoom(name: string, settings: RoomSettings, hostPlayer: Player) {
      ...
      this.state.roomsList = [...this.state.roomsList, newRoomInfo];

      // Initialize P2P Room
      this.initP2PRoom(roomId);
      this.broadcast();
  }
  ...
  public joinRoom(roomId: string, player: Player) {
      ...
      this.state.roomId = roomId;
      this.state.state = GameState.LOBBY;
      this.state.players = {}; // Will receive from host
      
      // Initialize P2P Room to connect to Host
      this.initP2PRoom(roomId);
      
      // Connection timeout watchdog
      setTimeout(() => {
          if (this.state.roomId === roomId && Object.keys(this.state.players).length === 0) {
              console.warn("Join timeout: No state received from host.");
              this.leaveRoom();
              // Remove dead room
              this.state.roomsList = this.state.roomsList.filter(r => r.id !== roomId);
              this.broadcast();
              alert("Host not responding. Room closed.");
          }
      }, 5000);
      
      // Wait a moment for connection then send Join
      setTimeout(() => {
          if (this.sendAction) {
              this.sendAction({ type: 'CLIENT_JOIN', player });
          }
      }, 1000);
  }

  private initP2PRoom(roomId: string) {
      try {
          if (this.room) {
              this.room.leave();
              this.room = null;
          ...
          const [send, get] = this.room.makeAction('gameAction');
          this.sendAction = send;
          
          this.room.onPeerJoin((peerId: string) => {
              console.log(`Peer ${peerId} joined`);
              this.peers[peerId] = true;
              // If I am host, I should send the full state immediately
              if (this.isHost) {
                  this.sendAction({ type: 'HOST_STATE', state: this.state });
              }
          });
          ...
          get((data: NetworkAction, peerId: string) => {
  ...
  public leaveRoom() {
      ...
      // 1. Notify network if possible
      if (this.sendAction && !wasHost) {
          this.sendAction({ type: 'CLIENT_LEAVE', playerId });
      }
      ...
      if (wasHost) {
          // Remove my room from the list locally
          this.state.roomsList = this.state.roomsList.filter(r => r.id !== roomId);
      }
  ...
  public playerMove(playerId: string, dx: number, dz: number) {
    ...
    // Client Logic: Send move to Host
    if (!this.isHost) {
        // Optimistic local update
        this.applyMove(playerId, dx, dz); 
        if (this.sendAction) {
            this.sendAction({ type: 'CLIENT_MOVE', playerId, x: dx, z: dz });
        }
        return;
    }
  ...
  private handleNetworkMessage(msg: NetworkAction, peerId: string) {
      if (this.isHost) {
          // HOST RECEIVES
          switch(msg.type) {
              case 'CLIENT_JOIN':
                  ...
                  // Immediately send back full state to sync the new joiner
                  this.sendAction({ type: 'HOST_STATE', state: this.state }); 
              ...
              case 'CLIENT_LEAVE':
                  ...
                  this.state.pot -= this.state.entryFee;
                  const r2 = this.state.roomsList.find(rm => rm.id === this.state.roomId);
                  if (r2) r2.playersCount = Object.keys(this.state.players).length;
                  this.broadcast();
                  break;
          }
      } else {
  ...
  private update(dt: number) {
    ...
    // Host broadcasts state to everyone via P2P
    if (this.isHost && this.sendAction) {
        // Optimization: only send essential dynamic data
        this.sendAction({ 
            type: 'HOST_STATE', 
            state: {
